...
import { Grid, Chip, Divider, Tabs, Tab, Stack, Typography, Paper } from '@mui/material';
import {
...
import { TimelineDotStyle, MarginPictures, generateTheme, ImageModal, API, IMG } from './util.jsx';
                    ...
                    <Grid item container textAlign='start' sx={{ mt: '30px' }}>
                        <Stack direction="row" spacing={5} >
                            ...
                            <Chip icon={selectedChip === 'Group' ? <DoneIcon /> : null}
                                ...
                                sx={{ '.MuiChip-label': { fontSize: 11, fontFamily: 'Helvetica' } }}  // 폰트 스타일링 적용
                            />
                            <Chip icon={selectedChip === 'Artfair' ? <DoneIcon /> : null}
                                label="ARTFAIR" onClick={() => btnData('Artfair')} style={{ backgroundColor: selectedChip === 'Artfair' ? '#eee' : 'transparent' }}
                                sx={{ '.MuiChip-label': { fontSize: 11, fontFamily: 'Helvetica' } }}  // 폰트 스타일링 적용
                            />
                        </Stack>
                    </Grid>
                    ...
                    <Grid item xs={12} textAlign='start'>
                        <Grid container>
                            <Grid item xs={3.5} textAlign='start' sx={{ mt: '112px' }}>
                                <StyledTypography>{selectedChip === 'Solo' ? 'SOLO EXHIBITION' : selectedChip === 'Artfair' ? 'ARTFAIR' : 'SELECTED & INVITATIONAL GROUP EXHIBITION'}</StyledTypography>
                                <StyledTypography>{data.length}</StyledTypography>
                            </Grid>
...
const UpcomingExhibition = ({ setSubTitle }) => {
    ...
    const fetchData = async () => {
        const response = await axios.get(`${IMG}/upcomingExhibition`)
    ...
    return (
        ...
        <>
            ...
            {isMobile ?
                ...
                // NonMobile
                <Grid container sx={{ mt: '10px', mb: '80px' }}>
                    <Grid item xs={1}></Grid>
