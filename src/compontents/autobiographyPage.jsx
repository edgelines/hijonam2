...
import { generateTheme, Share, IMG } from './util.jsx';
...
export const PostDetailView = ({ history, location, match, lang, componentRef, setCurrentPage }) => {
    ...
    const fetchData = async () => {
        try {
            const res = await axios.get(`${IMG}/autobiography/post/${postId}`);
            setData(res.data);
            setPost({
    ...
    const ViewsCount = async () => {
        try {
            const response = await axios.put(`${IMG}/autobiography/views/${postId}`); // 조회수 +1 증가
