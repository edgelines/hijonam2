...
import { Grid, Typography, Slider, Skeleton, TableContainer } from '@mui/material';
// import styledComponents from 'styled-components';
import axios from 'axios';
// import DoneIcon from '@mui/icons-material/Done';
import { generateTheme, MarginPictures, IMG } from './util.jsx';
...
export default function ArtworksPage() {
    ...
    const [imgSize, setImgSize] = useState(95);

    const someProps = { fontFamily: null, fontSize: null }
    const theme = generateTheme(someProps);
    const fetchData = async () => {
        try {
            const res1 = await axios.get(`${IMG}/genres`)
            setGenres(res1.data);

            const res2 = await axios.get(`${IMG}/artworks`);
            ...
            {isMobile ? (
                                ...
                                <SwiperSlide key={item.id}>
                                    <Grid item container direction="column" alignItems="center" style={{ textAlign: 'center' }} className="mx-auto" onClick={() => handleSelectedGenres(item.genres)}>
                                        <Grid item direction="column" alignItems="center" style={{ width: '100%' }}>
                                            <img src={`/img/Main/${item.fileName[0]}`} style={{ width: '100%', objectFit: 'cover', aspectRatio: '1/1', }} loading="lazy"
                ...
                <>
                    <Grid container sx={{ mt: '4vh' }}>
                        {genres.map((item) => (
                            <Grid item xs={1} container direction="column" alignItems="center" style={{ textAlign: 'center' }} className="mx-auto" key={item.id} onClick={() => handleSelectedGenres(item.genres)}>
                                <Grid item direction="column" alignItems="center" style={{ width: '100%' }}>
                                    <img src={`/img/Main/${item.fileName[0]}`} style={{ width: '8vw', objectFit: 'cover', aspectRatio: '1/1', }} className='rounded-3 mx-auto' />
                                </Grid>
                                <Grid item direction="column" alignItems="center" style={{ width: '100%' }}>
                    ...
                    <Grid container sx={{ mt: '1.5vh', mb: '140px' }}>
                        <Grid item xs={isLgTablet ? 2.6 : 2.1} sx={{ paddingLeft: isLgTablet ? 6 : 7.2, mt: -0.5 }}>
                            {selectedImg && selectedImg.title ?
                                <>
                                    <Grid container direction="column" alignItems="start">
                                        <h5 style={{ textAlign: 'start', fontFamily: 'Helvetica', fontWeight: 700 }}>{selectedImg.title}</h5>
                                    </Grid>
                                    <Grid container direction="column" alignItems="start" sx={{ mt: '-5px' }}>
                                        {selectedImg.title_kr ? <h5 style={{ textAlign: 'start', fontFamily: 'Nanum Gothic', fontWeight: 700 }}>{selectedImg.title_kr}</h5> : <></>}
                                    </Grid>
                                    <Grid container direction="column" alignItems="start">
                            ...
                            <Grid container>
                                {selectedImg && selectedImg.fileName && selectedImg.fileName.length > 1 ?
                                    selectedImg.fileName.map((item) => (
                                        <Grid item xs={4} key={item} sx={{ mb: 1 }} onClick={() => handleImgChange(item)}>
                                            <img src={`/img/Artworks/${item}`} loading="lazy"
                        ...
                        <Grid item xs={isLgTablet ? 6.2 : 6.7} >
                            {
                                selectedImg && selectedImg.fileName ?
