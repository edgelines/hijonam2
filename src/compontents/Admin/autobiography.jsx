...
export default function AutobiographyPage({ loadDataUrl }) {
    ...
    const saveToDatabase = async (newFormData) => {
        try {
            const response = await axios.post(`${IMG}/autobiography/save`, newFormData);
            setSeverity('success');
            setSnackbar(true);
    ...
    const updateDatabase = async (newFormData) => {
        try {
            ...
            const response = await axios.put(`${IMG}/autobiography/save/${newFormData.id}`, newFormData);
            setSeverity('success');
            setSnackbar(true);
            handlePageChange('Cancel')
        } catch (error) {
    ...
    const updateSequences = async () => {
        for (let i = data.length - 1; i >= 0; i--) {
            ...
            const item = data[i];
            try {
                const res = await axios.put(`${IMG}/${loadDataUrl}/autobiography/save/${item.id}`, {
                    ...item,
    ...
    const actionUploadEditorImage = async (file) => {
        ...
        try {
            ...
            const response = await axios.post(
                `${IMG}/autobiography/upload/Autobiography`, formData,
