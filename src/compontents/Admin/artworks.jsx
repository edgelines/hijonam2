...
import { AntSwitch, IMG } from '../util.jsx';
...
export default function ArtworksPage({ loadDataUrl }) {
    ...
    const [form, setForm] = useState({ fileName: "", genres: "", material: "", title: "", title_kr: "", sizeW: "", sizeH: "", executed: "", sequence: "" })
    // Form
    const newForm = () => { setEditMode(false); setDialog(true); resetForm(); }
    const resetForm = () => { setForm({ fileName: "", genres: "", material: "", title: "", title_kr: "", sizeW: "", sizeH: "", executed: "", sequence: "", showArtworks: "" }); };
    const editBtn = (content) => {
        setEditMode(true);
        setForm({ ...form, id: content.id, genres: content.genres, material: content.material, title: content.title, title_kr: content.title_kr, sizeW: content.sizeW, sizeH: content.sizeH, executed: content.executed, sequence: content.sequence, showArtworks: content.showArtworks, fileName: "", });
        setDialog(true);
    };
    ...
    const saveToDatabase = async (newFormData) => {
        ...
        try {
            ...
            const response = await axios.post(`${IMG}/${loadDataUrl}/Artworks`, formData, {
                headers: {
    ...
    const updateShowArtworks = async (item) => {
        try {
            const response = await axios.put(`${IMG}/${loadDataUrl}/Artworks/${item.id}`, {
    ...
    const updateSequences = async () => {
        for (let i = selectedData.length - 1; i >= 0; i--) {
            ...
            try {
                const res = await axios.put(`${IMG}/${loadDataUrl}/Artworks/${item.id}`, {
