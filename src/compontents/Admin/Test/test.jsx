...
import { Grid, Button, Snackbar, Alert, Typography, Dialog, DialogContent, DialogContentText, Input, DialogActions, TextField } from '@mui/material';
...
export default function HomeImageChagePage({ loadDataUrl }) {
    ...
    const url = 'http://hijonam.com/test/'
    ...
    const saveToDatabase = async () => {
        ...
        try {
            await axios.post(`${url}${loadDataUrl}2/Temp`, formData, {
                headers: {
    ...
    const updateDatabase = async () => {
        ...
        try {
            await axios.put(`${url}${loadDataUrl}2/Temp/${form.id}`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            });
    ...
    const fetchData = async () => {
        ...
        await axios.get(`${url}${loadDataUrl}2`).then((response) => {
    ...
    return (
        <Grid container>
            ...
            <Grid container>
                ...
                <Grid item xs={12}>
                    <Grid container>Test Table1</Grid>
                    <Grid container>
                        {data.map((item, index) => (
                            <Grid item xs={2} sx={{ padding: 5 }} key={item.id}>
                                <img src={`/img/Temp/${item.fileName}`} className="rounded-3 mx-auto"
                                    style={{
                                        width: '100%',
                                        aspectRatio: '1 / 1',
                                        objectFit: 'cover'
                                ...
                                />
                                <Grid container>
                                    <Grid item xs={12} sx={{ mt: '20px', borderBottom: '1px solid black' }}>
                                        {item.title}
                                    </Grid>
                                    <Grid item xs={6} sx={{ mt: 1 }}>
                                        <Button size="small" variant="contained" color="warning" onClick={() => editBtn(item)}>Edit</Button>
                                    </Grid>
                                    <Grid item xs={6} sx={{ mt: 1 }}>
                                        <Button size="small" variant="contained" color="error" onClick={() => deleteDatabase(item)}>Delete</Button>
                                    </Grid>
                                </Grid>
                            </Grid>
                        ))}
                ...
                <Grid item xs={12}>
                    <Grid container>Test Table2</Grid>
                    <Grid container>
                        {
                            data2.length > 0 &&
