...
export default function ExhibitionPage({ loadDataUrl, name }) {
    ...
    const saveToDatabase = async () => {
        ...
        try {
            const response = await axios.post(`${IMG}/${loadDataUrl}/Exhibition`, formData, {
    ...
    const updateDatabase = async () => {
        ...
        try {
            const response = await axios.put(`${IMG}/${loadDataUrl}/Exhibition/${form.id}`, formData, {
                headers: { 'Content-Type': 'multipart/form-data', },
            });
            setSnackbar(true);
            setSeverity('success');
                ...
                <DialogContent>
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={2}>
                            ...
                            >{[0, 1, 2].map((item) => (
                                <MenuItem value={item} key={item}>{item === 0 ? 'Solo' : item === 1 ? 'Group' : 'Artfair'}</MenuItem>
                            ))}
                            </TextField></Grid>
                        <Grid item xs={6}>
                            <TextField
                            ...
                            <TableBody>
                                {data.map((row) => (
                                    <TableRow key={row.id}>
                                        ...
                                        <TableCell>{row.soloGroup === 0 ? 'Solo' : row.soloGroup === 1 ? 'Group' : 'Artfair'}</TableCell>
