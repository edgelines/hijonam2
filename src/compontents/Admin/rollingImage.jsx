...
import { Grid, Button, Snackbar, Alert, Dialog, DialogContent, DialogContentText, Input, DialogActions } from '@mui/material';
...
export default function RollingImageChagePage({ loadDataUrl }) {
    ...
    const saveToDatabase = async () => {
        ...
        try {
            await axios.post(`${IMG}/${loadDataUrl}/Rolling`, formData, { headers: { 'Content-Type': 'multipart/form-data', }, });
            setSeverity('success');
            setSnackbar(true);
            // setDialog(false);
        } catch (error) {
    ...
    const updateDatabase = async () => {
        ...
        try {
            await axios.put(`${IMG}/${loadDataUrl}/Rolling/${form.id}`, formData, {
                headers: {
    ...
    const deleteDatabase = async (deleteID) => {
        ...
        if (confirmDelete) {
            try {
                await axios.delete(`${IMG}/${loadDataUrl}/Rolling/${deleteID.id}`);
                setSeverity('success');
                setSnackbar(true);
            } catch (error) {
                setSeverity('error');
