...
import { generateTheme, getFileType, ImageModal2, API, IMG, TEST } from './util.jsx';
...
export default function HomePage({ lang }) {
    ...
    const fetchData = async () => {
        await axios.get(`${IMG}/home`).then((response) => {
            var tmp = [];
        ...
        await axios.get(`${IMG}/upcomingExhibition`).then((response) => {
        ...
        await axios.get(`${IMG}/photos`).then((res) => {
        ...
        await axios.get(`${IMG}/autobiography`).then((response) => {
    ...
    return (
        <>
            {isMobile ? (
                ...
                <Grid container>
                    {/* 이미지 모달 */}
                    <ImageModal2 isOpen={isModalOpen} imageSrc={currentImage} width={'100vw'} onClose={() => setIsModalOpen(false)} />
                    ...
                    < Grid item xs={7} sx={{ height: '95svh' }
                        ...
                        <img src={`/img/Main/${imgData[0]}`}
                            onClick={() => handleImageClick(`/img/Main/${imgData[0]}`)}
                            style={{
                                top: 0,
                                left: 0,
                                width: '100%',
